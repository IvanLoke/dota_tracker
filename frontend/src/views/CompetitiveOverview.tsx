...
type StatsKey = 'Kills' | 'Deaths' | 'Assists' | 'Kill Participation' | 'GPM' | 'XPM';

const CompetitiveOverview = ({ stats }: CompetitiveOverviewProps): React.JSX.Element => {
    const { wins, loss, kills, deaths, assists, killParticipation, gpm, xpm } = stats;
    ...
    return <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-16">
            ...
            <GradientStats color="teal-400" twStyles="w-1/5">
                <p className="text-white font-bold">{wins} WINS</p>
            </GradientStats>
            <GradientStats color="red-500" twStyles="w-1/5">
                <p className="text-white font-bold">{loss} LOSSES</p>
            </GradientStats>
        </div>
        <div className="grid grid-cols-3 gap-4">
            {Object.keys(statsObject).map((key, i) => {
                return <GradientStats color="cyan-500" twStyles="w-5/6 p-2" key={`stats${i}`}>
                    <StatsValue 
                        statDescription={key} 
